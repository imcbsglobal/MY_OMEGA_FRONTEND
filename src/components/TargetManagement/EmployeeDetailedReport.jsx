...
import { getEmployeeDetailedReport } from '../../api/targetManagement';
import './targetManagement.css';

export default function EmployeeDetailedReport() {
  const { id } = useParams();
  ...
  useEffect(() => {
    (async () => {
      ...
      setLoading(true);
      try {
        const res = await getEmployeeDetailedReport(id);
        setData(res);
      } catch (err) {
        console.error(err);
          ...
          <div className="card">
            <div className="card-header pb-0">
              <div className="d-flex align-items-center justify-content-between">
                <h6>Employee Detailed Report</h6>
                <small className="text-muted">Employee ID: {id}</small>
              </div>
            </div>
