...
export default function RouteTargetReport() {
  const [employees, setEmployees] = useState([]);
  const [routes, setRoutes] = useState([]);
  const [filters, setFilters] = useState({ employee: '', route: '', start_date: '', end_date: '' });
  ...
  useEffect(() => {
    api.get('/employee-management/employees/?page_size=500')
      ...
      .catch(() => setEmployees([]));
    api.get('/target-management/routes/?page_size=500')
      .then(r => setRoutes(r.data.results || r.data || []))
      .catch(() => setRoutes([]));
  }, []);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const r = await api.get('/target-management/route-targets/', { params });
      ...
      data.forEach(target => {
        const base = {
          ...
          route: `${target.route_origin || ''} → ${target.route_destination || ''}`,
        ...
        const params = target.target_parameters || [];
        if (params.length === 0) {
      ...
      setRows(flat);
      setGenerated(true);
      if (flat.length === 0) toast.info('No route targets found for the selected filters.');
    } catch (e) {
      toast.error('Failed to fetch route target data.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExcel = () => {
    ...
    const headers = ['#', 'Employee', 'Employee ID', 'Route', 'Route Code', 'Period', 'Status', 'Parameter Type', 'Target Value', 'Achieved Value', 'Achievement %', 'Incentive (₹)'];
    const data = rows.map((r, i) => [
      i + 1, r.employee, r.employee_id, r.route, r.route_code, r.period, r.status,
      r.param_type, r.target_value, r.achieved_value, r.achievement_pct, r.incentive,
    ]);
    const ws = XLSX.utils.aoa_to_sheet([headers, ...data]);
    ws['!cols'] = [4, 20, 14, 24, 12, 22, 8, 24, 14, 14, 12, 12].map(w => ({ wch: w }));
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Route Target Report');
    XLSX.writeFile(wb, 'RouteTargetReport.xlsx');
  };
  ...
  const handlePrint = () => {
    ...
    win.document.write(`
      <html><head><title>Route Target Report</title>
      <style>
  ...
  const selectedRoute = routes.find(r => String(r.id) === String(filters.route));
          ...
          <div className="card">
            <div className="card-header pb-0">
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', alignItems: 'flex-end', padding: '0.75rem 1rem', background: '#f8fafc', borderRadius: 8, border: '1px solid #e5e7eb', marginBottom: '1rem' }}>
                ...
                <div style={{ flex: '0 0 auto', minWidth: 180 }}>
                  <label style={{ display: 'block', fontSize: '0.7rem', fontWeight: 600, color: '#6b7280', marginBottom: '0.35rem' }}>Route</label>
                  <select
                    className="form-select"
                    value={filters.route}
                ...
                <div style={{ flex: '0 0 auto' }}>
                  <button
                    ...
                    onClick={() => { setFilters({ employee: '', route: '', start_date: '', end_date: '' }); setRows([]); setGenerated(false); }}
            ...
            <div className="card-body" style={{ paddingTop: '0.5rem' }}>
              {generated && rows.length > 0 && (
                <>
                  ...
                  {/* Printable content */}
                  <div ref={printRef}>
                    <h2 style={{ textAlign: 'center', fontSize: 16, fontWeight: 700, marginBottom: 4 }}>Route Target Report</h2>
                    <p style={{ textAlign: 'center', color: '#555', marginBottom: 12, fontSize: 12 }}>
                      {selectedEmp ? `Employee: ${getEmpName(selectedEmp)}` : 'All Employees'}
                      {selectedRoute ? ` | Route: ${selectedRoute.origin} → ${selectedRoute.destination}` : ''}
                    ...
                    <div className="table-responsive" style={{ border: '1px solid #e2e8f0', borderRadius: 8 }}>
                      <table className="table align-items-center mb-0" style={{ fontSize: '0.78rem' }}>
                        <thead style={{ background: '#1e3a5f' }}>
                          <tr>
                            {['#', 'Employee', 'Emp ID', 'Route', 'Code', 'Period', 'Status', 'Parameter Type', 'Target', 'Achieved', 'Ach %', 'Incentive'].map(h => (
                        ...
                        <tbody>
                          {rows.map((r, i) => (
                            <tr key={i} style={{ background: i % 2 === 0 ? '#fff' : '#f5f8ff' }}>
                              ...
                              <td style={{ padding: '6px 10px', fontSize: '0.75rem' }}>{r.route}</td>
                              <td style={{ padding: '6px 10px', fontSize: '0.75rem', color: '#6b7280' }}>{r.route_code}</td>
                              ...
                              <td style={{ padding: '6px 10px', fontSize: '0.75rem', textAlign: 'right', fontWeight: 700, color: r.achievement_pct !== '-' ? pctColor(parseFloat(r.achievement_pct)) : '#6b7280' }}>
                                {r.achievement_pct}
                              </td>
              ...
              {generated && rows.length === 0 && !loading && (
                <div className="text-center text-muted py-4">No route targets found for the selected filters.</div>
              )}

              {!generated && (
                <div className="text-center text-muted py-5" style={{ fontSize: '0.9rem' }}>
                  Select filters and click <strong>Generate Report</strong> to view route target data.
                </div>
              )}
            </div>
          </div>
